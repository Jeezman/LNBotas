...
import { createLNMarketsService } from "./lnmarkets";
...
async function syncAllUserTrades() {
  ...
  try {
    ...
    console.log(`[SYNC SCHEDULER] Found ${usersWithCredentials.length} users with API credentials`);
    ...
    for (const user of usersWithCredentials) {
      try {
        console.log(`[SYNC SCHEDULER] Syncing trades for user ${user.username} (ID: ${user.id})`);
        ...
        try {
          ...
          console.log(`[SYNC SCHEDULER] Options sync failed for user ${user.username}:`, optionsError);
        }
        ...
        console.log(`[SYNC SCHEDULER] User ${user.username}: ${userSynced} new, ${userUpdated} updated`);
        
      } catch (userError) {
        console.error(`[SYNC SCHEDULER] Failed to sync trades for user ${user.username}:`, userError);
      }
    }

    console.log(`[SYNC SCHEDULER] Periodic sync completed: ${totalSynced} new trades, ${totalUpdated} updated trades across ${successfulSyncs} users`);
...
export function startPeriodicSync() {
  ...
  console.log(`[SYNC SCHEDULER] Periodic sync scheduled every ${SYNC_INTERVAL / 60000} minutes`);
}
